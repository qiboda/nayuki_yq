...
bool BoxApp::Initialize()
{
    ...
    NY_ASSERT_HR( mCommandList->Reset( mDirectCmdListAlloc.Get(), nullptr ) );
...
void BoxApp::Draw( const GameTimer& gt )
{
    ...
    NY_ASSERT_HR( mDirectCmdListAlloc->Reset() );

    // A command list can be reset after it has been added to the command queue via
    // ExecuteCommandList.
    // Reusing the command list reuses memory.
    NY_ASSERT_HR( mCommandList->Reset( mDirectCmdListAlloc.Get(), mPSO.Get() ) );
...
void BoxApp::BuildDescriptorHeaps()
{
    ...
    NY_ASSERT_HR( md3dDevice->CreateDescriptorHeap( &cbvHeapDesc, IID_PPV_ARGS( &mCbvHeap ) ) );
}
...
void BoxApp::BuildBoxGeometry(bool splitVertex)
{
    ...
    if (splitVertex)
    {
        NY_ASSERT_HR(D3DCreateBlob(vbByteSize, &mBoxGeo->VertexBufferCPU));
        CopyMemory(mBoxGeo->VertexBufferCPU->GetBufferPointer(), vertices.data(), vbByteSize);
    }
    else
    {
        NY_ASSERT_HR(D3DCreateBlob(vbpByteSize + vbcByteSize, &mBoxGeo->VertexBufferCPU));
        CopyMemory(mBoxGeo->VertexBufferCPU->GetBufferPointer(), verticesPoses.data(), vbpByteSize);
        CopyMemory((char*)mBoxGeo->VertexBufferCPU->GetBufferPointer() + vbpByteSize, verticesColors.data(), vbcByteSize);
    }

    NY_ASSERT_HR( D3DCreateBlob( ibByteSize, &mBoxGeo->IndexBufferCPU ) );
...
void BoxApp::BuildPSO()
{
    ...
    NY_ASSERT_HR( md3dDevice->CreateGraphicsPipelineState( &psoDesc, IID_PPV_ARGS( &mPSO ) ) );
}