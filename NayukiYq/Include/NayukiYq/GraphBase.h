...
class NAYUKIYQ_API GraphicsBase : public Singleton<GraphicsBase> {
    //--------------------
  ...
  private:
    ...
    VkResult GetQueueFamilyIndices(VkPhysicalDevice physicalDevice,
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(
            physicalDevice, &queueFamilyCount, queueFamilyPropertieses.data());
            ...
            if (surface)
                if (VkResult result = vkGetPhysicalDeviceSurfaceSupportKHR(
                    ...
                    std::cout << std::format(
                        ...
                        "queue family supports presentation!\nError code: {}\n",
                        int32_t(result));
                    return result;
                }
            // 若某队列族同时支持图形操作和计算
  ...
  public:
    ...
    VkResult GetPhysicalDevices() {
        ...
        if (VkResult result =
            ...
            std::cout << std::format(
                "[ graphicsBase ] ERROR\nFailed to get the count of physical "
                "devices!\nError code: {}\n",
                int32_t(result));
            return result;
        }
        if (!deviceCount)
            std::cout << std::format("[ graphicsBase ] ERROR\nFailed to find "
                                     "any physical device supports vulkan!\n"),
                abort();
    ...
    VkResult DeterminePhysicalDevice(uint32_t deviceIndex = 0,
        ...
        if (ig == notFound && enableGraphicsQueue ||
            ip == notFound && surface || ic == notFound && enableComputeQueue)
            return VK_RESULT_MAX_ENUM;
  ...
  private:
    ...
    VkResult CreateSwapchain_Internal() {
        ...
        if (VkResult result = vkGetSwapchainImagesKHR(
            ...
            std::cout << std::format(
                "[ graphicsBase ] ERROR\nFailed to get the count of swapchain "
                "images!\nError code: {}\n",
                int32_t(result));
            return result;
        }
  ...
  public:
    ...
    VkResult RecreateSwapchain() {
        ...
        if (result) {
            std::cout << std::format(
                "[ graphicsBase ] ERROR\nFailed to wait for the queue to be "
                "idle!\nError code: {}\n",
                int32_t(result));
            return result;
        }
  ...
  public:
    ...
    VkResult UseLatestApiVersion() {
        // 因为vkEnumerateInstanceVersion需要版本至少为1.1,
        // 所以调用vkGetInstanceProcAddr，先进行检查。
