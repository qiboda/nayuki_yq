...
Logger::Get(const LoggerCategory &loggerCategory) {
...
void Logger::RegisterCategory(const LoggerCategory &loggerCategory) {
    ...
    case LoggerCategory::Type::Sync: {
        if (Get(loggerCategory) == nullptr) {
            ...
            std::cerr << "sync category already exist this category:  "
                      << loggerCategory.GetName() << ", duplication register!"
                      << std::endl;
        }
        break;
    }
    case LoggerCategory::Type::Async: {
        if (Get(loggerCategory) == nullptr) {
            ...
            std::cerr << "async category already exist this category:  "
                      << loggerCategory.GetName() << ", duplication register!"
                      << std::endl;
        }
        break;
...
std::shared_ptr<spdlog::logger>
Logger::CreateSyncLogger(const LoggerCategory &loggerCategory) {
    try {
        auto console_sink =
        ...
        auto file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
            "./" + loggerCategory.GetName() + ".log", true);
        if (file_sink) {
            file_sink->set_level(spdlog::level::trace);
        }

        spdlog::sinks_init_list sinksInitList{console_sink, file_sink};
        auto syncLogger = std::make_shared<spdlog::logger>(
            loggerCategory.GetName(), sinksInitList);
        if (syncLogger) {
...
std::shared_ptr<spdlog::logger>
Logger::CreateAsyncLogger(const LoggerCategory &loggerCategory) {
    try {
        auto console_sink =
            std::make_shared<spdlog::sinks::ansicolor_stdout_sink_mt>();
        if (console_sink) {
        ...
        auto file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
            "./" + loggerCategory.GetName() + ".alog", true);
        if (file_sink) {
            file_sink->set_level(spdlog::level::trace);
        }
        ...
        auto asyncLogger = std::make_shared<spdlog::async_logger>(
            loggerCategory.GetName(), sinksInitList, spdlog::thread_pool(),
