...
module meta_forge.command_list_parser;
...
static llvm::cl::opt<std::string> TargetName( llvm::cl::Positional,
                                              llvm::cl::desc( "<target name>" ),
                                              llvm::cl::Required,
...
CommandListParser::CommandListParser( int& argc, const char** argv )
{
    ...
    // 收集文件列表
    mTargetName = TargetName.getValue();
...
//     {
//         if ( llvm::StringRef( path ).ends_with( ".ixx" ) )
//         {
