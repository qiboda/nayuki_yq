...
#include <Core/Delegate/DelegateInterface.h>
...
struct TimerData {
    ...
    bool IsValid() const { return timerState != TimerState::None; }
};
...
class CORE_API TimerManager : public Singleton<TimerManager> {
  ...
  private:
    struct TimerHandleExpireTimeGreater {
        bool operator()(const TimerHandle &LHS, const TimerHandle &RHS) const {
            const TimerData &timerDataLHS =
                TimerManager::GetInstance().GetTimerData(LHS);
            const TimerData &timerDataRHS =
                TimerManager::GetInstance().GetTimerData(RHS);
  ...
  private:
    ...
    using TimerDataHashMap =
        std::unordered_map<TimerHandle, TimerData, TimerHandle::Hash>;
  ...
  public:
    TimerHandle SetTimer(f32 intervalTime, TimerDelegate timerDelegate,
                         bool bLoop = false, bool bActive = true);
