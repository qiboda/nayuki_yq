...
class CORE_API MathHelper
{
  public:
    ...
    static f32 RandF() { return static_cast<f32>( rand() ) / static_cast<f32>( RAND_MAX ); }

    // Returns random f32 in [a, b).
    static f32 RandF( f32 a, f32 b ) { return a + RandF() * ( b - a ); }

    static i32 Rand( i32 a, i32 b ) { return a + rand() % ( ( b - a ) + 1 ); }
    ...
    template <typename T> static T Lerp( const T &a, const T &b, f32 t ) { return a + ( b - a ) * t; }
