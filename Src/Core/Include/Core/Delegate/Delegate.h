...
#pragma region Add
    ...
    DelegateHandle
    Add(typename DelegateFunInstance<TReturnVal, TArgs...>::TFunctor functor) {
        DelegateHandle delegateHandle(
            DelegateHandle::GenerateNewHandleType::GenerateNewHandle);
        DelegateInstanceType Instance =
            new DelegateFunInstance<TReturnVal, TArgs...>(functor);
    ...
    DelegateHandle
    AddMem(typename DelegateMemFunInstance<TClass, TReturnVal,
                                           TArgs...>::TFunctor functor,
           TClass *obejct) {
        DelegateHandle delegateHandle(
            DelegateHandle::GenerateNewHandleType::GenerateNewHandle);
        DelegateInstanceType Instance =
            new DelegateMemFunInstance<TClass, TReturnVal, TArgs...>(functor,
                                                                     obejct);
        Instance->SetHandle(delegateHandle);
        mDelegateInstances.push_back(Instance);
        return delegateHandle;
    ...
    template <typename TClass>
    DelegateHandle
    AddSP(typename DelegateSPMemFunInstance<TClass, TReturnVal,
                                            TArgs...>::TFunctor functor,
          std::shared_ptr<TClass> SPObejct) {
        DelegateHandle delegateHandle(
            DelegateHandle::GenerateNewHandleType::GenerateNewHandle);
        DelegateInstanceType Instance =
            new DelegateSPMemFunInstance<TClass, TReturnVal, TArgs...>(
                functor, SPObejct);
        Instance->SetHandle(delegateHandle);
        mDelegateInstances.push_back(Instance);
        return delegateHandle;
...
#pragma region Remove
    ...
    bool Remove(DelegateHandle delegateHandle) {
        auto remove_it = std::find_if(
            mDelegateInstances.begin(), mDelegateInstances.end(),
...
#pragma region ThreeFive
    ...
    SingleDelegate(const SingleDelegate &singleDelegate) noexcept
        : mDelegateInstance(singleDelegate.mDelegateInstance == nullptr
                                ? nullptr
...
#pragma region Bind
    ...
    template <typename TClass>
    DelegateHandle
    BindMem(typename DelegateMemFunInstance<TClass, TReturnVal,
                                            TArgs...>::TFunctor functor,
            TClass *obejct) {
    ...
    template <typename TClass>
    DelegateHandle
    BindSP(typename DelegateSPMemFunInstance<TClass, TReturnVal,
                                             TArgs...>::TFunctor functor,
           std::shared_ptr<TClass> SPObejct) {
        NY_ASSERT_MSG(mDelegateInstance == nullptr,
                      L"SingleDelegate only binds once");
...
#pragma region Unbind
    ...
    bool Unbind(DelegateHandle delegateHandle) {
        if (mDelegateInstance &&
            mDelegateInstance->GetHandle() == delegateHandle) {
            return Unbind();
        }
        return false;
    }
