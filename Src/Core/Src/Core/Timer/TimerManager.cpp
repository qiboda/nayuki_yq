    ...
    for ( auto executeTimerHandle : pendingExecuteTimerHandles )
    {
        if ( executeTimerHandle.IsValid() )
        {
            ...
            if ( timerData.bloop )
            {
                ...
                executeCount += static_cast<i32>( elapsedTime / timerData.intervalTime );
            }
            ...
            if ( timerData.bloop )
            {
                ...
                timerData.expireTime = timerData.expireTime + static_cast<f32>( executeCount ) * timerData.intervalTime;
...
TimerHandle TimerManager::SetTimer( f32 intervalTime, TimerDelegate timerDelegate, bool bLoop, bool bActive )
{
    TimerData timerData;
